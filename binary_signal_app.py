...
def send_telegram_photo(photo_path, caption=""):
    ...
    with open(photo_path, 'rb') as photo:
        ...
        try:
            ...
            st.warning(f"Telegram photo error: {e}")
...
# ─── MAIN ──────────────────────────────────────────────
st.title("🤖 Multi-Symbol Forex Bot | Env Vars | Telegram Alerts")
...
for symbol in selected_symbols:
    ...
    st.metric(f"{symbol} Accuracy", f"{ml_accuracy:.2f}%", delta=f"Session: {session_acc:.2f}%")
    ...
    if st.button(f"🕰️ Backtest {symbol}", key=symbol):
        ...
        if st.checkbox(f"📩 Send {symbol} summary", key=f"summary_{symbol}"):
            msg = (
